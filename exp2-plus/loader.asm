...
BaseOfLoader              equ 0a000h  	; LOADER.BIN 被加载到的位置 ---- 段地址
...
ReadSector:
    ...
    mov    word [si + 0], 0x10   ; buffer_packet_size
...
;     mov    bp, sp
;     pusha    
;     ; 不使用bx, dx进行输出操作, cx作为中转寄存器, di作为列号
;     and    al, 0x0f             ; 取寄存器后4位
;     cmp    al, 9                
;     jnc    .Char                ; 大于9,为字母
; .Num:
...
FindLoaderInRootDir:
    mov    eax, [RootDirSectorNow]; ax <- 现在正在搜索的扇区号(从19开始)
