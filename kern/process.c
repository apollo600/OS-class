...
free_pid(u32 old_pid) {
	assert(old_pid <= PID_COUNT);
	// 插入表中
	u32 old_index = pid_map[0];
	u32 old_next_index = pid_map[old_index];
	pid_map[old_index] = old_pid;
	pid_map[old_pid] = old_next_index;
}
